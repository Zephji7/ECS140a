...
public class Scanner {
	...
	public Token getNextToken() {
		...
		if (tmp.getText().equals("#")) {
			...
			if (peekNextToken().getText().equals("\n")) {
				tokenBuf.remove(0);
				tmp = peekNextToken();
			}
		}
	...
	private l1charType l1CharCheck(int char_ascii) {
		...
		// WhiteSpace beside Space(32)
				...
				//TODO: mark newline character
				if (char_ascii == 10) {
					return l1charType.NewLine;
				}
	...
	private l2charType l2CharCheck(int char_ascii) {
		...
		int[] operator = {40, 41, 42, 43, 44, 45, 47, 58, 59, 60, 61, 62, 123, 125};
	...
	private l3charType l3CharCheck(int char_ascii) {
		int[] logicOperator = {58, 60, 61, 62, 33};
	...
	private boolean charBufHandling() {
		...
		if (charBuf.size() == 0) {
			...
			// skip whitespace
			// TODO:
			else if ((nexCharl1Type == l1charType.WhiteSpace) || (nexCharl2Type == l2charType.Space) || ((nexCharl1Type == l1charType.Backslash) && (this.fileStream.peekNextChar() == 10))) {
				if ((nexCharl1Type == l1charType.Backslash) && (this.fileStream.peekNextChar() == 10)) {
					this.fileStream.getNextChar();
				}
				return false;
			}
			...
			// start with operator
			else if ((nexCharl2Type == l2charType.Operator) || (nexCharl1Type == l1charType.NewLine)) {
		...
		// If char buffer is NOT empty---> Compares its' type to see if it matches
			...
			// start with operator
				...
				if ((charBuf.get(0).l3Type == l3charType.Negative) && (this.pre_token_type != tokenType.IDENTIFIER) && (this.pre_token_type != tokenType.INT_CONSTANT) && (this.pre_token_type != tokenType.FLOAT_CONSTANT) && (nexCharl2Type == l2charType.Digits)) {
					// consume next char
					this.fileStream.getNextChar();
					// update the nexchar linenum and position
					theChar.linenum = this.fileStream.getLinenum();
	...
	private void strHandling() {
		...
		if (this.next_token_type != tokenType.NONE) {
			...
			for (int i = 0; i < charBuf.size(); i++) {
				//TODO: better way to handle double quote in STRING_CONSTANT
				// don't add the double quote to the string
				if (charBuf.get(i).l1Type != l1charType.DubQuote) {
		...
		if (this.next_token_type == tokenType.IDENTIFIER) {
			...
			for (int i = 0; i < this.keywordList.size(); i++) {
				if (tmptext.equals(this.keywordList.get(i))) {
					kyflag = true;
				}
			}
			if (kyflag) {
			...
			else if (tmptext.equals("_")) {
				this.finTokenType = Token.TokenType.EMPTY_IDENTIFIER;
			}
			else {
				if (tmptext.indexOf('_') != -1) {
